...
const problems = [];
problems.push({
    collectionId: 'fw8746dncj4kfin',
    collectionName: 'temporary_problems',
    created: '2024-08-03 23:48:34.972Z',
    description: 'Write a method to find and return the index of an int searchFor in an array of ints arr. If the value is not found return -1. Happy Coding!',
    id: 'p5wog0u9uzbm1c4',
    inputs: {
      arr: { java: 'int[] arr', cpp: 'int arr[]', python: 'arr' },    
      searchFor: {
        java: 'int searchFor',
        cpp: 'int searchFor',
        python: 'searchFor'
    ...
    method_name: 'searchForInt',
    point_value: 6,
    return_type: { java: 'int', cpp: 'int' },
    testcase_in: {
      '1': { public: 'true', arr: '{3, 4, 5, 6}', searchFor: '4' },   
      '2': { public: 'false', arr: '{6, 2, 1}', searchFor: '3' },     
      '3': {
        public: 'true',
        arr: '{13, 19, 16, 107, 4096}',
        searchFor: '4096'
      }
    },
    testcase_out: { '1': '1', '2': '-1', '3': '4' },
    time_limit: 0,
    title: 'Search For Number in Array',
    updated: '2024-08-08 16:55:04.194Z',
    userid: 'wkhu1cp26kwnvod'
  });
  [0]
  let casesJson = {}
    let count  = 0;
            for(let i = 1;; i++){
                if(problems[0].testcase_in[i+""] === undefined){
                    break;
                }
                if(problems[0].testcase_in[i+""].public === 'true'){
                    count++;
                    casesJson[count] = problems[0].testcase_in[i+""];
                    casesJson["out"+count] = problems[0].testcase_out[i+""];
                    // casesJson[count].public = undefined;
                }
            }
const textProblems = [];
textProblems.push({title: problems[0].title, text: problems[0].description, points: problems[0].point_value, testcases: casesJson})
...
io.on('connection', (socket) =>{
    ...
    socket.on('stop', (msg) => {
        const index = socketIDs.indexOf(msg.id);
        runSockets[index].close();
        socketIDs.splice(index);
        runSockets.splice(index);
    })
    socket.on('run', (msg) => {
        ...
        //     console.log(runSyntax);
            if(socketIDs.indexOf(msg.id) === -1){
            const runnerSocket = new WebSocket(runnerEndpoint);
            runSockets.push(runnerSocket)
            socketIDs.push(msg.id);
            ...
            runnerSocket.addEventListener('open', () => {
                ...
                runnerSocket.on('message', (msg2) => {
                    ...
                    //     outputCount++;
                    // }
                    console.log("Sending to " + "terminal-"+msg.id);
                    console.log(msg2.toString());
                    
                    io.emit("terminal-"+msg.id, {text: msg2.toString()})
...
// 
    ...
    socket.on("canJoin", (msg) => {
        console.log("Recieved Join Request");
        if(users.indexOf(msg.username) !== -1){
            socket.send({value: false})
        }else{
            socket.send({value: true, user: msg.username})
        }
    })
    ...
    socket.on('add', (msg) =>{
        ...
        if(index !== -1 && docs[index] !== undefined){
            ...
            io.emit(msg.id, {type: 'add', atChar: msg.atChar+offset, text: msg.text, username: msg.username, number: msg.editNumber, language: msg.language});
    ...
    socket.on('remove', (msg) =>{
        ...
        if(index !== -1 && docs[index] !== undefined){
            ...
            //3. Send the change to all users on the document
            io.emit(msg.id, {type: 'remove', startChar: msg.startChar, endChar: msg.endChar, username: msg.username, number: msg.editNumber, language: msg.language});
