...
function FullEditorPage() {
  const navigate = useNavigate();
    ...
    const [terminalObjects, setTerminalObjects] = useState([]);
    ...
    const [tabSelected, setTabSelected] = useState("terminal");
    ...
    useEffect(() => {
      setTeamname(Cookies.get("username"))
      setUser2(Cookies.get("username"))
      setSessionID2(Cookies.get("code") + "\\" + currentProblemNum)
    ...
    useEffect(() => {
      try{
        setSessionID(Cookies.get("code") + "\\" + currentProblemNum);
        sessionIDRef.current = (Cookies.get("code") + "\\" + currentProblemNum);
        setUser(user2);
    ...
    useEffect(() => {
      if(socket !== undefined){
        ...
        console.log("terminal-"+sessionID)
        socket.on("terminal-"+sessionID, (msg) => {
          ...
          const termObj = <TerminalObject time={`${hours}:${minutes}:${seconds}`} text={msg.text}/>
          setTerminalObjects((prevObjects) => [...prevObjects, termObj]);
        })
  ...
  <button onClick={(event) =>{
          ...
          // hole\1 FullEditorPage.js:301
          // CODE\1 FullEditorPage.js:302
          // hole FullEditorPage.js:303
          // hole FullEditorPage.js:304
          // hole FullEditorPage.js:306
          // text/x-java
  ...
  setUser2(event.target.value)
      ...
      <img src={whiteLogo} className = 'logo1'/> 
      ...
      <div className='editor-bottom'>
        ...
        <div className='editor-left'>
          {/*  */}
          {/*  */}
          {/* You can find the old code at https://docs.google.com/document/d/1hzPkoY4JG3RpTfA8-G2UUym2eiGJ8s4MmLtJS0obw-A/edit?tab=t.0 */}
          {/* <Console/> */}
          <div className='problem-window'>
            <div className='problem-window-top'>
              ...
              <h2 className='problem-window-top-headings'   onClick={()=>{setTabSelected('terminal')}}
                style={tabSelected === 'terminal' ? selectedTabStyle.current : unselectedTabStyle.current}>
                  Terminal
              </h2>
              {/* <h2 className='problem-window-top-headings'   onClick={()=>{setTabSelected('users')}}
                style={tabSelected === 'users' ? selectedTabStyle.current : unselectedTabStyle.current}>
                  Users
              </h2> */}
              <h2 className='problem-window-top-headings'   onClick={()=>{setTabSelected('info')}}
                style={tabSelected === 'info' ? selectedTabStyle.current : unselectedTabStyle.current}>
                  Session Info
              </h2>
            ...
            <div className='problem-window-bottom problem-text' style={{
                    display: (tabSelected === 'info' ? 'flex' : 'none'),
                    position: 'relative'
                  }}>
            ...
            <div className='problem-window-bottom problem-text' style={{
                    display: (tabSelected === 'users' ? 'flex' : 'none'),
                    position: 'relative'
                  }}>
                    <h1 className='session-title'>
                      Users
            ...
            <div className='problem-window-bottom' style={{
                    display: (tabSelected === 'terminal' ? 'flex' : 'none'),
                    position: 'relative'
                  }}>
                  ...
                  {/* <h1 className='session-title problem-title'>
                      Terminal
                    </h1> */}
                  <div className='newTerminal'>
                    {terminalObjects.map((obj) => obj)}
                  ...
                  {caseIsRunning ? (<div className='running-button'>
                    ...
                    if(runnerFunction.current !== undefined){
                      runnerFunction.current(currentProblemNum, 'run');
                    }
                  }}>
                    Run
                  </div>)}
                  {caseIsRunning ? (<div/>) : (<div className='stop-button' onClick={() => {
                    if(runnerFunction.current !== undefined){
                      runnerFunction.current(currentProblemNum, 'stop');
                    }
                  }}>
                    Stop
                  </div>)}
        ...
        <div className='editor-right'>
          ...
          <div className='editor-editor'>
            <EditorStateManager caseSetter={setTestCaseElements} addLine={addLineFunction.current} sessionID={sessionID} user={Cookies.get("username")} setSocket={handleSocket} currentLanguage={currentLanguage} loadingSetter1={setEditorIsLoading} problemsSetter={setProblems} runnerFunction={runnerFunction}/>
